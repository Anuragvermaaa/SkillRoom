...
const UserForgotPassword = ({ closeModal, showToast }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await submitForgotPassword({ email, type: "user" });
      showToast("OTP sent to your email!", "success");
      setErrors({});
      localStorage.setItem("forgotEmail", email);
      console.log(`Navigating to /users/verify-otp with email: ${email}`);
      setTimeout(() => {
        navigate("/users/verify-otp", { state: { email }, replace: true });
        closeModal();
