...
const BusinessVerifyOtp = ({ closeModal, showToast }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!email) {
      ...
      navigate("/business/forgot-password", { replace: true });
      return;
    }
    setIsLoading(true);
    try {
      const response = await verifyOtp({ otp, email, type: "business" });
      ...
      setErrors({});
      console.log(`Navigating to /business/reset-password with email: ${email}, token: ${token}`);
      setTimeout(() => {
        navigate("/business/reset-password", { state: { email, token }, replace: true });
        closeModal();
      }, 300);
    } catch (err) {
      setIsLoading(false);
